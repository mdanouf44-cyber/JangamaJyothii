        ...
        <div className="flex space-x-2">
          {Array.from({ length: totalSlides }).map((_, index) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-all duration-300 ${Math.floor(currentSlide / 2) === index ? 'bg-green-600 scale-125' : 'bg-gray-300 hover:bg-gray-400'}`}
            />
          ))}
        </div>
        <button
      ...
      <div className="flex flex-wrap justify-center gap-2 mt-6">
        {varieties.map((variety, index) => (
          <button
            ...
            className={`px-3 py-2 rounded-full text-sm font-medium transition-all duration-300 flex items-center gap-2 ${currentSlide <= index && index < currentSlide + 2 ? 'bg-green-600 text-white shadow-lg scale-105' : 'bg-white text-gray-700 border border-gray-300 hover:bg-green-50 hover:border-green-300'}`}
          >
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              Discover our premium cardamom varieties with detailed information
            </p>
          </div>
